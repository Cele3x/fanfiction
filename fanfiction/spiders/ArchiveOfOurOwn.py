...
def find_story_status(text: str, reviewed_on: datetime) -> [str, None]:
    """Find story status inside HTML snippet.

    :param text: str
            unformatted status text; e.g. "11/18"
           reviewed_on: datetime
            last story update

    :return: str or None
        for the story status
    """
    text_split = text.split('/')
    if len(text_split) != 2:
        return None
    p1 = str_to_int(text_split[0])
    p2 = str_to_int(text_split[1])
...
class ArchiveOfOurOwnSpider(CrawlSpider, ABC):
    ...
    def parse_story(self, response, user_url, story_url):
        ...
        reviewed_on = story_meta_sel.xpath('.//dt[contains(text(), "Updated:")]/following-sibling::dd[1]/text()').get()
        story_meta.add_value('reviewedOn', get_date(reviewed_on))
        chapter_status = story_meta_sel.xpath('.//dt[contains(text(), "Chapters:")]/following-sibling::dd[1]/text()').get()
        status = find_story_status(chapter_status, get_date(reviewed_on))
