...
class ArchiveOfOurOwnMissingSpider(CrawlSpider, ABC):
    name = 'ArchiveOfOurOwnMissing'
    download_delay = 7.5
    allowed_domains = ['archiveofourown.org']

    custom_settings = {
        # 'JOBDIR': 'crawls/AO3-Missing',
    }

    def start_requests(self):
        ids = [ObjectId('62ec23c59107bc230d06f80b'), ObjectId('62ec23cf9107bc230d06f81a'), ObjectId('62e5b0e3b4a7f454e93048b0'), ObjectId('62f0ba1e2305c4be8e24560e'), ObjectId('62e55ff3b4a7f454e92ff638'), ObjectId('62e1a0711338c6207ba8adde')]
        items = self.db.chapters.find({'_id': {'$in': ids}})
        items_count = self.db.chapters.count_documents({'_id': {'$in': ids}})
        ...
        with tqdm(total=items_count) as pbar:
            for item in items:
                ...
                if 'url' in item:
                    ...
                    yield Request(url=item['url'], callback=self.parse_chapters, cb_kwargs=dict(story_url=item['url'], story=story))
    ...
    def parse_chapters(self, response, story_url, story=None):
        ...
        if not chapters:  # only 1 chapter
            ...
            loader.add_value('storyUrl', story['url'])
